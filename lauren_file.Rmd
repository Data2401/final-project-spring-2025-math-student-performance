...
- What is the relationship between alcohol consumption (weekdays Dalc vs weekends Walc) and final grades?
```{r}
formatted %>% group_by(weekend_alcohol) %>% mutate(AvgFinalGrade= mean(final_grade)) %>% count(AvgFinalGrade)
formatted %>% group_by(workday_alcohol) %>% mutate(AvgFinalGrade= mean(final_grade)) %>% count(AvgFinalGrade)
```
...
formatted %>% ggplot(aes(x= as.factor(failures), y= period1_grade)) +
  geom_boxplot(outlier.shape = NA) + geom_jitter(color= "slateblue", size=2, alpha=1/2)+
  labs(
...
formatted %>% ggplot(aes(x= as.factor(failures), y= period2_grade)) +
  geom_boxplot(outlier.shape = NA) + geom_jitter(color= "hotpink1", size=2, alpha=1/2)+
  labs(
    x = "Number of Past Failures",
    y = "Period 2 Grade", 
...
formatted %>% ggplot(aes(x= as.factor(failures), y= final_grade)) +
  geom_boxplot(outlier.shape = NA) + geom_jitter(color= "springgreen2", size=2, alpha=1/2)+
  labs(
    x = "Number of Past Failures",
    y = "Final Grade", 
...
ggplot(avg_grades_long, aes(x = Period, y = Average_Grade, group = 1)) +
  ...
  labs(title = "Average Math Grades Over Time",
       x = "Grade Period",
       y = "Average Grade") +
  theme_minimal()
```
