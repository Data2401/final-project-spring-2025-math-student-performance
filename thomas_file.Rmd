...
-   **DT** to display the data table prettier. 🌈
...
formatted %>%
  ...
  labs(
    x = "Final Grade", y = "Parents' Edu. Level",
    title = "Parents' Combined Education Level vs. Student Final Grade"
  ) +
  coord_cartesian(ylim = c(2.57, 9.25), xlim = c(0,20))
```

```{r fig.align='center'}
formatted %>% #Family no relation
  ggplot(aes(x = factor(family_support), y = period3_grade, fill = family_relationship)) + geom_boxplot() + theme_linedraw()
```

```{r fig.align='center'}
formatted %>% #Internet helps studying
  ggplot(aes(x = factor(study_time), y = period3_grade, fill = internet_use)) + geom_boxplot() + theme_linedraw() #Internet good 4 studying
```
...
long_grade_data %>% ggplot(aes(x = factor(study_time), y = grade)) + geom_boxplot(fill = "skyblue") +
  facet_wrap(~ period) +
  labs(x = "Study Time:   1 = <2hrs,   2 = 2-5hrs,   3 = 5-10hrs,   4 = >10hrs", y = "Grade", title = "Grades vs. Weekly Study Time Per Period") + theme_linedraw()
```

```{r fig.align='center'}
formatted %>% #More study time is better for overall grade trend, sharp drop for (4), maybe overconfidence or too much studying lowers retention of key information.
  select(period1_grade, period2_grade, period3_grade, study_time) %>%
  pivot_longer(cols = starts_with("period"), names_to = "period", values_to = "grade") %>%
  group_by(study_time, period) %>%
  summarise(mean_grade = mean(grade)) %>%
  ggplot(aes(x = period, y = mean_grade, group = study_time, color = factor(study_time))) +
  geom_line(size = 1.2) +
  geom_point() +
  labs(x = "Term", y = "Average Grade", color = "Study Time (Hours)", title = "Grade Average Trend by Study time") +
  theme_linedraw()
```
...
```{r fig.align='center'}
formatted %>% 
  ggplot(aes(x = factor(romantic), y = period3_grade)) + geom_boxplot() #w/ go out w/ friends
```
...
- Ask to undo factorization of family edu so simplify plot code
- Delete (fix) legend in family ridge plot
